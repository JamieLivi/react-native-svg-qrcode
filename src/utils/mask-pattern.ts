...
export function isValid(mask: number): boolean {
  return mask != null && mask !== undefined && !isNaN(mask) && mask >= 0 && mask <= 7;
}
 ...
 * @return {number | undefined}           Valid mask pattern or undefined
 */
export function from(value: number | string): number | undefined {
  return isValid(Number(value)) ? parseInt(value.toString(), 10) : undefined;
}
